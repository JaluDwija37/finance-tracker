...
function GrowthPlot({ values }: { values: { label: string; netWorth: bigint }[] }) {
  ...
  return <div className="report-growth-plot"><svg viewBox="0 0 720 225" role="img" aria-label={`Grafik total aset dari ${values[0].label} sampai ${values[values.length - 1].label}`}><path className="report-grid-line" d="M34 40H686M34 115H686M34 190H686"/><path className="report-growth-line" d={line}/>{points.map((point) => <circle key={point.label} cx={point.x} cy={point.y} r="5" aria-label={`${point.label}: ${money(point.value)}`}/>)}</svg><div className="report-plot-labels"><span>{values[0].label}</span><span>{values[values.length - 1].label}</span></div></div>;
}
